...
namespace IC.SDK.Serialization
{
    ...
    public class Serializer : IStreamSerializer, IBytesArraySerializer
    {
        ...
        public Serializer()
        {
            ...
            Logger.Log.Debug("Serializer. Ctr. Exit");
        }
        ...
        public void Serialize(Stream stream, object obj, TcpSerializeTypes serializeType)
        {
            try
            {
                ...
                {
                    var propertyValue = _objectProperties.GetPropertyValue(obj, property.Name);
        ...
        /// <returns></returns>
        public object Deserialize(Stream stream, string dataLayerNamespaceName, string dataLayerLibName)
        {
            ...
            try
            {
                ...
                foreach (TcpSerializeTypes type in Utils.GetValues(new TcpSerializeTypes()))
                {
                    if (byteType == (byte)type)
                    {
                        Logger.Log.Debug("Deserialize. Exit");
                        return _readFromStream.ReadObject(stream, type, dataLayerNamespaceName, dataLayerLibName);
                    }
                }
        ...
        public object DeserializeFromBytesArray(byte[] bytesArray)
        {
            ...
            try
            {
                ...
                foreach (UdpSerializeTypes type in Helpers.Utils.GetValues(new UdpSerializeTypes()))
                {
                    if (byteType == (byte)type)
                    {
                        Logger.Log.Debug("DeserializeFromBytesArray. Exit");
                        return _readFromBytesArray.ReadObjectFromBytesArray(bytesArray, type);
                    }
                }

                throw new ArgumentException(
                    string.Format("DeserializeFromBytesArray. Unknown id of enum type: {0}", byteType));
            }
            catch(Exception ex)
            {
