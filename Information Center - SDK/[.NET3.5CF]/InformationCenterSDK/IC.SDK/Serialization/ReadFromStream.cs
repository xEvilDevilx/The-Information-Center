...
namespace IC.SDK.Serialization
{
    ...
    /// </summary>
    public class ReadFromStream : IReadFromStream
    {
        #region Variables
        ...
        public byte[] ReadBytesArray(Stream stream)
        {
            ...
            try
            {
                ...
                {
                    var count = stream.Read(bytesArray, bytesRead, 
                        bytesArray.Length - bytesRead);
                    if (count == 0)
                    {
        ...
        public DateTime ReadDateTime(Stream stream)
        {
            ...
            try
            {
                ...
                var dateTime = new DateTime(BitConverter.ToInt64(bytesArray, 0), 
                    DateTimeKind.Local); 
                var timeString = dateTime.ToString().Replace(" PM", "");
