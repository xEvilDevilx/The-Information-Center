...
namespace IC.Client.Network
{
    ...
    internal class NetworkTransfer : INetworkTransfer
    {
        ...
        /// <summary>IC BytesArray Serialization tool</summary>
        ...
        public void UDPProcess(byte[] bytesArray)
        {
            ...
            try
            {
                var ipAddressString = (string)_bytesArraySerializer.
                    DeserializeFromBytesArray(bytesArray);
            ...
            try
            {
                ...
                if (obj is SystemTranslationData)
                {
                    if (CurrentLanguage == "CN")
                        _mainEntry.ControlsPlugin.SetFont(CurrentLanguage);
                    else _mainEntry.ControlsPlugin.SetDefaultFont();
        ...
        private void SetDefaultLangCurr(DefaultLangCurr defaultLangCurr)
        {
            ...
            if (!string.IsNullOrEmpty(defaultLangCurr.EmptyLang))
            {
                DefaultLanguage = defaultLangCurr.EmptyLang;
                CurrentLanguage = defaultLangCurr.EmptyLang;
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(defaultLangCurr.EmptyCurrency))
            {
                DefaultCurrency = defaultLangCurr.EmptyCurrency;
                CurrentCurrency = defaultLangCurr.EmptyCurrency;
            }
            else
            {
