...
namespace IC.Client.FormModel
{
    ...
    public class PluginManager : IPluginManager
    {
        ...
        public PluginManager(Control.ControlCollection controls)
        {
            ...
            var controlPluginDirName = string.Format("{0}\\Plugins\\{1}.dll", 
                Utils.GetCurrentDirectory(), LoadPluginName(PluginTypes.Control));
            var hardwarePluginDirName = string.Format("{0}\\Plugins\\{1}.dll",
                Utils.GetCurrentDirectory(), LoadPluginName(PluginTypes.Hardware));
            ControlsDictionary.Clear();
            LoadAssemblies(controlPluginDirName);
            LoadAssemblies(hardwarePluginDirName);
        ...
        /// <summary>
        /// Use for Copy all plugin Fonts
        /// </summary>
        private void CopyFonts()
        {
            Logger.Log.Debug("CopyFonts. Enter");

            try
            {
                var currPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().
                    GetName().CodeBase);
                var path = string.Format("{0}\\Plugins\\Fonts\\", currPath);
                var files = Directory.GetFiles(path);
                if (files.Length > 0)
                {
                    for (int i = 0; i < files.Length; i++)
                    {
                        var cleanPath = files[i].Replace(path, "");
                        var destFile = string.Format("Windows\\{0}", cleanPath);
            ...
            {
                Logger.Log.Error("CopyFonts. ", ex);
            }

            Logger.Log.Debug("CopyFonts. Exit");
        }

        /// <summary>
        /// Use for Set Font to all Plugin Control text
        /// </summary>
        /// <param name="language">Language string</param>
        public void SetFont(string language)
        {
            Logger.Log.Debug("SetFont. Enter");

            try
            {
                var chinaFontName = "Arial Unicode MS";
                switch (language)
                {
                    case "ENG":
                        break;
                    case "CN":
                        foreach (var control in ControlsDictionary)
                            control.Value.SetFont(chinaFontName);
                        break;
                    case "RUS":
                        break;
                }
            ...
            {
                Logger.Log.Error("SetFont. ", ex);
            }

            Logger.Log.Debug("SetFont. Exit");
        }

        /// <summary>
        /// Use for Set Default Font to all Plugin Control text
        /// </summary>
        public void SetDefaultFont()
        {
            Logger.Log.Debug("SetDefaultFont. Enter");

            try
            {
                foreach (var control in ControlsDictionary)
                    control.Value.SetDefaultFont();
            }
            catch (Exception ex)
            {
                Logger.Log.Error("SetDefaultFont. ", ex);
            }

            Logger.Log.Debug("SetDefaultFont. Exit");
        }
                ...
                foreach (Type type in assembly.GetTypes())
                {
                    if (type.IsClass)
                    {
                        ...
                        foreach (Attribute attribute in attributes)
                        {
                            ...
                            var hardwareTypeAttr = attribute as HardwareTypeAttribute;
                            if ((hardwareTypeAttr != null) && (KeyHook == null))
                            {
                                var keyHook = (IKeyHook)Activator.CreateInstance(type);
                                KeyHook = keyHook;
                            }
                        }
                    }
            ...
            try
            {
                ...
                switch (pluginType)
                {
                    case PluginTypes.Control:
                        ...
                        break;
                    case PluginTypes.Hardware:
                        pluginName = FormModelConstants.ConfigHardwarePluginName;
                        break;
                }
